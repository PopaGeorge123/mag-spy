...
import { updateBellState }from "@/libs/API/alerts";

export default function AlertStateButton({ alertId , enabled }) {
  const [bellEnabled, setBellEnabled] = useState(enabled);
  ...
  return (
    <button onClick={
      async () => {
        await updateBellState(alertId, !bellEnabled);
        setBellEnabled(!bellEnabled);
    }
    }>
      {bellEnabled ? enabledIcon() : disabledIcon()}
    </button>
  )
}