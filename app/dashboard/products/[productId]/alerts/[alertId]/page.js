...
export default async function EditAlertPage({ params:{ alertId , productId } }) {
  ...
  const isAvailable = product.isActive;
    ...
    isAvailable ? (<div className='w-full'>
      ...
      <h1 className="text-xl m-5">Edit <span className='text-white text-2xl'>{alertData.name}</span></h1>
      ...
      <form action={EditAlert}  className="max-w-sm mx-auto">
        ...
        <div className="mb-5">
          <label className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Rename your alert</label>
        ...
        <button type="submit" className="text-white bg-blue-500 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Update</button>
      </form>
